...
pub fn find_relative_ranks(score: Vec<i32>) -> Vec<String> {
    let mut priority_queue = std::collections::BinaryHeap::from(score.clone());
    let rank_map = (0..score.len()).fold(std::collections::HashMap::new(), |mut map, i| {
        map.insert(
    ...
    return score
        .iter()
        .map(|score| rank_map[&score].to_string())
        .collect();
}
