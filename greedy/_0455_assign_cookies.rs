...
pub fn find_content_children(g: Vec<i32>, s: Vec<i32>) -> i32 {
    ...
    let mut sorted_cookie = s.clone();
    sorted_cookie.sort_unstable();
    let mut child: i32 = 0;
    for i in 0..sorted_cookie.len() {
        if sorted_g[child as usize] <= sorted_cookie[i] {
            child += 1;
        }
        if child as usize >= sorted_g.len() {
            break;
        }
    }
    return child;
